...
class _JobControlTaskPanel:
    ...
    def write_input_file_handler(self):
        ...
        if self.check_prerequisites():
            QApplication.setOverrideCursor(Qt.WaitCursor)
            try:
                import CalculiXInpWriter

                ccx_writer = CalculiXInpWriter.writer(self.TempDir, self.MeshObject, self.FixedObjects, self.ForceObjects, self.MaterialObject)
                self.Basename = ccx_writer.write_calculix_input_file()
            except:
