...
class _ResultControlTaskPanel:
    ...
    def vm_stress_selected(self, state):
        ...
        QApplication.setOverrideCursor(Qt.WaitCursor)
        self.MeshObject.ViewObject.setNodeColorByScalar(self.result_object.ElementNumbers, self.result_object.StressValues)
    ...
    def select_displacement_type(self, disp_type):
        QApplication.setOverrideCursor(Qt.WaitCursor)
        if disp_type == "Uabs":
            self.MeshObject.ViewObject.setNodeColorByScalar(self.result_object.ElementNumbers, self.result_object.DisplacementLengths)
        else:
            match = {"U1": 0, "U2": 1, "U3": 2}
            d = zip(*self.result_object.DisplacementVectors)
            displacements = list(d[match[disp_type]])
            self.MeshObject.ViewObject.setNodeColorByScalar(self.result_object.ElementNumbers, displacements)
    ...
    def show_displacement(self, checked):
        ...
        self.MeshObject.ViewObject.setNodeDisplacementByResult(self.result_object)
...
def reset_mesh_color(mesh=None):
    ...
    mesh.ViewObject.setNodeColorByScalar()
