...
class FemTest(unittest.TestCase):
    ...
    def create_new_mesh(self):
        self.mesh_object = self.active_doc.addObject('Fem::FemMeshObject', 'Mesh')
    ...
    def test_new_analysis(self):
        ...
        self.active_doc.saveAs("/tmp/auto_test.fcstd")
        FreeCAD.Console.PrintMessage('\nChecking FEM inp file write...\n')
        fea.setup_working_dir('/tmp/FEM')
        error = fea.write_inp_file()
        # md5sum of src/Mod/Fem/test_file.inp
        md5_standard = "4de49f1f9ef63e56f180bb42f7b55ccf"
        import hashlib
        md5_test = hashlib.md5(open('/tmp/FEM/Mesh.inp', 'rb').read()).hexdigest()
        self.assertEqual(md5_standard, md5_test, "FemTools write_inp_file failed. md5 sums don't match")
